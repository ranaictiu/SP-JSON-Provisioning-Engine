function $_global_clientforms() {
    ...
    if (typeof spWriteProfilerMark == 'function')
        spWriteProfilerMark("perfMarkBegin_" + "clientforms.js");
    SPClientForms = {};
    ...
    SPClientForms.ClientFormManager.SubmitClientForm = function(qualifier) {
        if (SP != null && SP.QoS != null) {
            SP.QoS.WriteUserEngagement("ClientForm_Save_Click");
        }
        ...
        this.SubmitClientForm = function() {
            ...
            function CompleteClientRequest() {
                ...
                if (redirectInfo.popUI && window.frameElement != null && typeof window.frameElement.commitPopup == "function") {
                    var dialogRedirectUrl = Nav.getUrlKeyValue('DialogRedirectUrl');

                    if (dialogRedirectUrl != null && dialogRedirectUrl != '')
                        window.frameElement.commitPopup(dialogRedirectUrl);
                    else {
                        var rteRetVal = {
                            newFileUrl: _ctx.ItemAttributes.Url,
                            isFolder: _ctx.ItemAttributes.FsObjType == 1,
                            newFileSize: _ctx.ItemAttributes.FileSize,
                            newFileIcon: _ctx.ItemAttributes.FileIcon
                        };

                        window.frameElement.commitPopup(rteRetVal);
                    }
                    return;
                }
                ...
                source = source == null || source == '' ? Nav.getUrlKeyValue("NextPage") : source;
                source = source == null || source == '' ? Nav.getUrlKeyValue("NextUsing") : source;
                if (Boolean(source)) {
    ...
    if (typeof spWriteProfilerMark == 'function')
        spWriteProfilerMark("perfMarkEnd_" + "clientforms.js");
}
var SPClientForms;
...
function SPFieldNote_Edit(rCtx) {
    ...
    function BuildEnhancedRichTextHtml() {
        ...
        result += ' ' + STSHtmlEncode(_myData.fieldSchema.Title);
        if (_myData.fieldSchema.Required)
            result += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
...
function SPFieldFile_Edit(rCtx) {
    ...
    if (_isFile) {
        ...
        result += '<input type="text" value="' + STSHtmlEncode(fileName) + '" maxlength="251" id="' + STSHtmlEncode(_inputId) + '" ';
        result += 'title="' + STSHtmlEncode(_myData.fieldSchema.Title);
        if (_myData.fieldSchema.Required)
            result += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
        result += '" class="ms-input" size="35" />';
        ...
        result += '<input type="text" value="' + STSHtmlEncode(_initialValue) + '" maxlength="256" id="' + STSHtmlEncode(_inputId) + '" ';
        result += 'title="' + STSHtmlEncode(_myData.fieldSchema.Title);
        if (_myData.fieldSchema.Required)
            result += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
        result += '" class="ms-long" size="35" />';
        result += '<br /></span>';
        return result;
    }
    return '';
...
function SPFieldChoice_Dropdown_Edit(rCtx) {
    ...
    function BuildFillInChoiceHtml() {
        ...
        if (_myData.fieldSchema.Required)
            fTitle += ' ' + Strings.STS.L_RequiredField_Text;
    ...
    function BuildSelectHtml() {
        var valueSet = false;
        var select = '<select id="' + STSHtmlEncode(_dropDownId) + '" title="' + STSHtmlEncode(_myData.fieldSchema.Title);

        if (_myData.fieldSchema.Required)
            select += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
        select += '" class="ms-RadioText">';
        if (_isChoiceEmpty)
...
function SPFieldChoice_Radio_Edit(rCtx) {
    ...
    function BuildRadioChoiceHtml() {
        ...
        if (_isFillInChoice) {
            ...
            if (_myData.fieldSchema.Required)
                fTitle += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
    ...
    for (var j = 0; j < _selectedChoices.length; j++) {
        ...
        if (typeof _allChoicesDict[_selectedChoices[j]] == "undefined") {
            if (_isFillInChoice && _allChoicesDict[_constFillIn] == _constFillIn)
                ...
                _allChoicesDict[_constFillIn] = _allChoicesDict[_constFillIn] + ";#" + _selectedChoices[j];
        }
        else {
...
function SPFieldDateTime_Edit(rCtx) {
    ...
    function BuildDateInput() {
        ...
        dateResult += "title=\"" + STSHtmlEncode(_myData.fieldSchema.Title);
        if (_myData.fieldSchema.Required)
            dateResult += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
        dateResult += "\" class=\"ms-input\" AutoPostBack=\"0\" /></td>";
        ...
        dateResult += "<img id=\"" + _datePickerImgId + "\" src=\"" + "/_layouts/15/images/calendar_25.gif?rev=44" + "\" border=\"0\" ";
        dateResult += "alt=\"" + STSHtmlEncode(Strings.STS.L_DateTimeFieldSelectTitle) + "\"></img></a></td>";
        dateResult += "<td><iframe id=\"" + _datePickerIframeId + "\" src=\"" + "/_layouts/15/images/blank.gif?rev=44" + "\" frameborder=\"0\" ";
...
function SPFieldUrl_Edit(rCtx) {
    ...
    if (_myData.fieldSchema.Required)
        result += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
    result += "\" class=\"ms-long\" " + _styleStr + "/><br />";
...
function SPFieldUser_Display(rCtx) {
    ...
    function GetPicture(value, userSchema) {
        ...
        if (value.picture == null || value.picture == '') {
            if (_spPageContextInfo.crossDomainPhotosEnabled) {
                ...
                r += "/_layouts/15/images/person.gif?rev=44";
            }
            r += '" alt="';
            r += STSHtmlEncode(StBuildParam(Strings.STS.L_UserFieldPictureAlt1, value.title));
            if (_myData.fieldSchema.Required)
                r += ' ' + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
            r += '" />';
        }
        else {
    ...
    function BuildUserTitle(value, userSchema) {
        ...
        r += GetPresence(value, userSchema, true);
        r += UserLink(value);
        r += STSHtmlEncode(value.title.replace(/\,\,/g, ','));
        r += '</a></span>';
        return r;
    }
...
function SPClientPeoplePickerCSRTemplate(rCtx) {
    ...
    var _notificationSpanId = _topSpanId + '_NotificationSpan';
    ...
    _myData.fieldSchema.NotificationElementId = _notificationSpanId;
    ...
    if (_myData.fieldSchema.AllowOnlyEmailAddresses) {
        initialHelpText = Strings.STS.L_SPClientPeoplePickerMultiUserEmailOnlyHelpText;
    }
    ...
    var topspanCSS = _myData.fieldSchema.InPlaceEditMode ? "ms-grid" : "sp-peoplepicker-topLevel";
    var topspanIPEStyle = _myData.fieldSchema.InPlaceEditMode ? 'style="padding-left:3px;padding-top:1px;cursor:text;height:100%;border-width:1px;position:relative;min-width:' + (_myData.fieldSchema.Width - 8).toString() + 'px"' : '';
    var inlineHelpTxt = _myData.fieldSchema.InPlaceEditMode ? "" : initialHelpText;
    var inputEditorIPEStyle = _myData.fieldSchema.InPlaceEditMode ? 'style="margin:0px"' : '';
    var result = '<div dir="' + STSHtmlEncode(_myData.fieldSchema.Direction) + '">';

    result += '<div class="' + topspanCSS + '" id="' + STSHtmlEncode(_topSpanId) + '" title="' + _titleAttr + '"' + topspanIPEStyle + '/>';
    ...
    result += 'ms-helperText">' + STSHtmlEncode(inlineHelpTxt) + '</span>';
    ...
    result += '"/_layouts/15/images/gears_anv4.gif?rev=44"' + '/>';
    result += '<span id="' + STSHtmlEncode(_resolveListId) + '" class="sp-peoplepicker-resolveList"></span>';
    result += '<span id="' + STSHtmlEncode(_notificationSpanId) + '" aria-live="polite" style="visibility:hidden;position:absolute;width:0px;display:inline-block;"></span>';
    result += '<input type="text" class="sp-peoplepicker-editorInput" size="1" autocomplete="off" value="" ';
    result += 'id="' + STSHtmlEncode(_editorId) + '" title="' + _titleAttr + '"' + inputEditorIPEStyle + ' autocorrect="off" autocapitalize="off" /></div>';
    result += '</div></div>';
...
function SPFieldAttachments_Default(rCtx) {
    ...
    if (_myData.fieldValue != '') {
        ...
        for (var i = 0; i < allAttachments.length; i++) {
            ...
            if (_myData.controlMode == SPClientTemplates.ClientControlMode.EditForm) {
                ...
                result += '" src="';
                result += "/_layouts/15/images/rect.gif?rev=44";
    ...
    function BuildAttachmentsUploadControl() {
        ...
        upload += "/_layouts/15/images/rect.gif?rev=44";
        upload += '"/>';
