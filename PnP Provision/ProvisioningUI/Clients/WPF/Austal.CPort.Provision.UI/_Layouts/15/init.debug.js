...
function ExpCollGroup(groupName, imgName, evt, noAjax) {
    ...
    var fOpen = false;
    var displayStr;
    var bUpdateImgAltText = typeof ExpFlighting != "undefined" && ExpFlighting.UseGU30();
...
function _AddTrySPListNextBanner() {
    var bannerHtml = '\
<div class="ms-fullWidth" style="background-color:#F8F8F8;position:relative;">\
  ...
  <div class="ms-tableCell ms-verticalAlignTop ms-fullWidth" style="padding:16px">\
    <h2 style="color:#444">' + STSHtmlEncode(Strings.STS.L_DocLibNewLookHeading) + '\
...
function SPThemeUtils_module_def() {
    ...
    function UseClientSideTheming() {
        ...
        var useCST = featureEnabled && typeof Theming !== strUndefined && !ClientSideThemingOptOut();
    ...
    function ClientSideThemingOptOut() {
        var cstOptOut = false;
        var currentWebTemplate = GetWebTemplate();

        switch (currentWebTemplate) {
    ...
    function ApplyCurrentTheme(hideContentWhileTheming) {
        ...
        if (FlightingAvailable() && Flighting.VariantConfiguration.IsExpFeatureClientEnabled(367) && ClientSideThemingOptOut()) {
            RemoveAllElementsWithId(idThemingCurtain);
            if (UseCdnForCss()) {
                RestoreCssLinksToOriginalUrls();
            }
            return;
        }
        if (UseClientSideTheming()) {
